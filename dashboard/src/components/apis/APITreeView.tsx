...
interface Props {
  endpoints: Endpoint[];
  onSelect: (endpoint: Endpoint, method: Method) => void;
  ...
  return (
    <TreeView<APITreeItemType>
      ...
      onPrimaryAction={(items) => {
        if (items.data.data && items.data.method) {
          onSelect(items.data.data, items.data.method);
        }
      }}
