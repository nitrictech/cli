...
const providerRegex = `\w+\/\w+@` + semverRegex
...
func getProviderFromProviderString(fullProvider string) (*provider, error) {
...
func FromFile(cfc types.ConfigFromCode, name, provider string, envMap map[string]string, opts *types.ProviderOpts) (types.Provider, error) {
	...
	prov, err := getProviderFromProviderString(sc.Provider)
	if err != nil {
		return nil, err
	}

	// If its a nitric provider it will be downloaded automatically if not already available
